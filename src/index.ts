...
import { checkOllamaHealth, runLocalAgent } from './local-agent.js';
import { classifyPrivacy } from './privacy-filter.js';
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  // --- Local model routing ---
  // Groups with modelType='local' bypass containers entirely
  if (group.modelType === 'local') {
    ...
    logger.info(
      ...
      'Routing to local agent',
    );

    await channel.setTyping?.(chatJid, true);
    const result = await runLocalAgent(group, prompt, chatJid, channel);
  ...
  // --- Privacy filter (non-main Claude groups only) ---
  if (!isMainGroup && PRIVACY_FILTER_ENABLED) {
    const rawContent = missedMessages.map((m) => m.content).join('\n');
    const classification = await classifyPrivacy(rawContent);
    ...
    if (classification.verdict !== 'normal') {
      logger.info(
        {
          group: group.name,
          layer: classification.layer,
        ...
        'Privacy filter: routing to local Qwen',
      );
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          // Local-model groups don't have containers — always enqueue for processGroupMessages
          if (group.modelType === 'local') {
